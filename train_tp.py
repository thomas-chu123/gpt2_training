...
from transformers import Trainer, TrainingArguments, AdamWeightDecay
import torch

def main():
    torch.cuda.empty_cache()
    train_test_dataset = (load_dataset('json', data_files='source/train_tp.json',split='train')).train_test_split(test_size=0.2)
    ...
    train_ids = train_test_dataset['train'].map(case_tokenize, batched=True, remove_columns=['case'])
    test_ids = train_test_dataset['test'].map(case_tokenize, batched=True, remove_columns=['case'])
